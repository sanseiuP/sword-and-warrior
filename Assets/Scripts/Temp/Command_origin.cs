...
public class Command_origin : MonoBehaviour
{ /*
    //GameObject[] obj;
    ...
    {
        int move_direction=InputManager_origin.getInstance().getMoveInput();
    ...
    public void Attack(Warrior player)
    {
        ...
        bool Is_Attacking=InputManager_origin.getInstance().getAttackInput();
        if (Is_Attacking) {
            player.setWave();
        }
    }
    ...
    {
        bool Is_Swap_Weapon=InputManager_origin.getInstance().getSwapWeaponInput();
        if (Is_Swap_Weapon) {
